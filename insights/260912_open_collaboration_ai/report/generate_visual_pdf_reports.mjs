...
function renderFigure(key, title, tables, notes, edition, assets) {
  ...
  if (key === "01") {
    ...
    const context = tables[1];
    visual = `<div class="reviewer-compare">${comparison.rows.map((row, index) => `<article data-kind="${index === 0 ? "agent" : "user"}"><h4>${inline(row[0])}</h4><strong>${inline(row[3])}</strong><p>${inline(row[2])} / ${inline(row[1])} PRs received another commit</p><i style="--review-rate:${percentage(row[3]) ?? 0}%"></i></article>`).join("")}</div>${context ? `<div class="review-context">${context.rows.map((row) => `<article><strong>${inline(row[2])}</strong><h4>${inline(row[0])}</h4><p>${inline(row[1])}</p></article>`).join("")}</div>` : ""}`;
  } else if (key === "12B.1") {
  ...
  const noteHtml = notes.length
    ...
    : "";
  return `<section class="figure-block" data-figure="${key}"><header class="figure-heading"><div class="figure-kicker">${esc(edition.figure)} ${esc(key)}</div><h3>${inline(title.replace(/^(?:Figure|图)\s+[^·]+·?\s*/i, ""))}</h3></header>${visual}${noteHtml}</section>`;
}
...
function css(fonts, language) {
  return `
    ...
    *{box-sizing:border-box}html{background:var(--paper)}body{margin:0;background:var(--paper);color:var(--ink);font-family:${language === "zh" ? '"Alibaba Report","PingFang SC",sans-serif' : '"Helvetica Neue","Alibaba Report",Arial,sans-serif'};font-size:9.4pt;font-weight:400;line-height:1.58;-webkit-print-color-adjust:exact;print-color-adjust:exact}a{color:#5941e7;text-decoration:none}strong,b{font-weight:700}code{padding:.08em .28em;background:#ece9e2;border-radius:2px;font-family:ui-monospace,SFMono-Regular,Menlo,monospace;font-size:.84em}p{margin:0 0 3.2mm;color:#44433f;orphans:3;widows:3}ul{margin:1mm 0 4mm;padding-left:5mm}li{margin:0 0 1.2mm}hr{margin:9mm 0;border:0;border-top:1px solid var(--line)}
    ...
    .front-page{break-after:page}.front-page h2{margin:0 0 7mm;font-size:25pt}.scope-grid{display:grid;grid-template-columns:repeat(2,1fr);border-top:1px solid var(--ink);border-left:1px solid var(--line)}.scope-grid article{min-height:36mm;padding:5mm;border-right:1px solid var(--line);border-bottom:1px solid var(--line)}.scope-grid strong{display:block;color:var(--signal);font-size:28pt;line-height:1}.scope-grid span{display:block;margin-top:3mm;color:var(--quiet)}.contents-list{margin-top:9mm;display:grid;gap:0;border-top:1px solid var(--ink)}.contents-list h2{margin:4mm 0}.contents-list a{padding:2.3mm 1mm;display:grid;grid-template-columns:12mm 1fr auto;gap:3mm;border-bottom:1px solid var(--line);color:var(--ink)}.contents-list b{color:var(--signal)}.contents-list small{color:var(--quiet)}
    ...
    .metric-cards{display:grid;grid-template-columns:repeat(3,1fr);gap:4mm}.metric-cards article{min-height:57mm;padding:5mm;background:#fff;border:1px solid var(--line)}.metric-cards article>span{color:var(--signal);font-size:7pt;font-weight:700}.metric-cards article>strong{display:block;margin:5mm 0 3mm;color:var(--signal);font-size:29pt;line-height:1}.metric-cards h4{min-height:16mm}.metric-cards p{font-size:7pt}.reviewer-compare{display:grid;grid-template-columns:repeat(2,1fr);gap:5mm}.reviewer-compare article{min-height:61mm;padding:6mm;background:var(--signal-soft);border:1.3px solid var(--ink)}.reviewer-compare article[data-kind="user"]{background:var(--model-soft)}.reviewer-compare h4{margin:0 0 7mm;font-size:12pt}.reviewer-compare strong{display:block;color:var(--signal);font-size:34pt;line-height:.9}.reviewer-compare article[data-kind="user"] strong{color:#397fb9}.reviewer-compare p{margin:4mm 0;font-size:8pt}.reviewer-compare i{display:block;height:3mm;background:linear-gradient(90deg,var(--signal) 0 var(--review-rate),#e8e6e0 var(--review-rate))}.reviewer-compare article[data-kind="user"] i{background:linear-gradient(90deg,#397fb9 0 var(--review-rate),#e8e6e0 var(--review-rate))}.review-context{display:grid;grid-template-columns:repeat(3,1fr);margin-top:5mm;border-top:1px solid var(--line);border-bottom:1px solid var(--line)}.review-context article{padding:4mm;border-left:1px solid var(--line)}.review-context article:first-child{border-left:0}.review-context strong{font-size:18pt}.review-context h4{margin:2mm 0;font-size:8pt}.review-context p{margin:0;color:var(--quiet);font-size:7pt}
